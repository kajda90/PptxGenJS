...
| `legendLayout`  | object  |         |           | positioning legend within chart area | object with `x`, `y`, `w` and `h` props, all in range 0-1 (proportionally related to the chart size). Ex: `{x: 0, y: 0, w: 1, h: 1}` fully expands legend to the chart area |
